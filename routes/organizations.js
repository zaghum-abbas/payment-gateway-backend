...
const { addOrganization, getAllOrganizations, getOrganizationTransactions } = require('../controllers');
const { createPaymentLink, updateTransaction, editPaymentLink, deletePaymentLink, deleteOrganization, updateOrganizationStatus } = require('../controllers/organizations');
const { authorizeOrg } = require('../middleware/auth.middleware');
const router = express.Router();

router.post('/add-organization', addOrganization);
router.get('/', getAllOrganizations);
router.patch('/:organization_id/status', updateOrganizationStatus);
